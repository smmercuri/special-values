...
\section{Rescaling by a rational}
\label{sec:rescale}

Deligne's period is only ever defined up to $E(M)^{\times}$ --- it is a determinant of a
comparison isomorphism computed in rational bases, and changing the bases rescales it --- and
the incomplete $L$-function differs from the complete one by a rational at a critical integer.
The conjecture is stated as a membership precisely so as to be invariant under both. The
following lemmas make this rigorous, and they are what licenses every ``up to $\Q^{\times}$''
in an instance.
...
\begin{remark}[Why the hypotheses are restricted to the critical set]
  ...
  package is free to carry whatever value it likes. Two packages asserting the same conjecture
  may therefore disagree arbitrarily off the critical set --- including in whether $c^{+}$
  ...
  false $\sigma(v) = 2v^{\sigma}$. That is why the Galois-invariance hypothesis is there, and
  why the two halves are separate lemmas with different hypotheses rather than one.
\end{remark}
...
\begin{proof}
  ...
  Definition~\ref{def:pkg}, (viii) and (ix) hold componentwise, (ix) because $\sigma_{*}$ on subfields
  is a left adjoint (to $\sigma^{*}$) and therefore preserves joins, which is mathlib's
  \lean{IntermediateField.map\_sup}; (x) because a compositum of algebraic intermediate fields
  lies in $\Qbar$;
  and (xi) because a product of nonzero periods is nonzero.
