...
\begin{remark}[Why (xi) is part of the definition, and why nondegeneracy is not]
  ...
  parity, since by Definition~\ref{def:gl1-critical} the critical sets of an even and an odd
  character are disjoint --- so the requirement is not preserved by $\oplus$.
\end{remark}
...
\begin{definition}[Nondegeneracy]
  ...
  \uses{def:isCritical}
  An object $M$ is \emph{nondegenerate} if it has at least one critical integer. This is a
  predicate on the mathematics and is not given a Lean name: an instance states it directly,
  as $\exists n,\ \mathrm{IsCritical}(M,n)$.
\end{definition}

\begin{theorem}[The degenerate case is vacuously true]
  \label{thm:vacuous-conjecture}
  \lean{Deligne.Pkg.conjecture\_of\_forall\_not\_isCritical}
  \leanok
