...
\begin{lemma}[The field of values is algebraic]
  ...
  Every element of $\Q(\chi)$ is algebraic over $\Q$, which is requirement (x) of
...
Remark~\ref{rem:not-a-pkg}: Lemma~\ref{lem:gl1-period-ne-zero} discharges requirement (xi)
and Lemma~\ref{lem:values-algebraic} requirement (x), both fields of the structure, so without
...
\begin{lemma}[Nondegeneracy: a critical integer exists]
  ...
  \lean{Deligne.GL1.exists\_isCritical, Deligne.GL1.exists\_isCritical\_pkg}
  \leanok
...
\begin{lemma}[Requirement (xi): the period does not vanish]
...
\begin{proof}
  ...
  The statement is deliberately stronger than requirement (xi) demands --- it drops the
...
\begin{lemma}[Hurwitz zeta at non-positive integers, interior form]
  ...
  \[ \zeta_H(x,-k) \;=\; \frac{-1}{k+1}\, B_{k+1}(xi). \]
\end{lemma}
\begin{proof}
  ...
  $B_1(xi) = x - 1/2$ (\lean{Polynomial.bernoulli\_one}) finishes.
...
\begin{proof}
  ...
  whatever the parity of $k$. So the criticality of $1-n$ for $\chi^{-1}$ --- which does hold,
  the functional equation exchanging the two halves of the critical set --- is never needed.
\end{proof}
...
\begin{lemma}[Galois conjugation preserves parity and transports the field of values]
  ...
  requirements (viii) and (ix) of Definition~\ref{def:pkg} for the package below.
\end{lemma}
\begin{proof}
  \leanok
...
\begin{definition}[The GL(1) package]
  ...
  Lemma~\ref{lem:gl1-crit-pkg}. Of the requirements of Definition~\ref{def:pkg}, (vii) is
  trivial, the weight being constant, (viii) and (ix) are Lemma~\ref{lem:gl1-galois-transport},
  (x) is Lemma~\ref{lem:valueField-algebraic}
  and (xi) is Lemma~\ref{lem:gl1-period-ne-zero}; nondegeneracy is
...
\begin{lemma}[The package's Gamma factor is mathlib's]
  ...
  \lean{DirichletCharacter.gammaFactor} $\chi\, s$, and the weight of every object is $0$.
\end{lemma}
\begin{proof}
  \leanok
